...
export const SUBSCRIBE_COMPLETED_TASKS = gql`
  query subscribeCompletedTasks($after: timestamptz! = "${START_OF_TODAY}", $before: timestamptz! = "${END_OF_TODAY}") {
    tasks(
      where: {
        _and: [
          { start: { _gte: $after } },
...
export function useCompletedTasks() {
  ...
  const { loading, error, data, fetchMore, variables } = useSubscription(SUBSCRIBE_COMPLETED_TASKS, options);
